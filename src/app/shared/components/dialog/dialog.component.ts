...
@Component({
  ...
  template: `
    ...
    @if (config.zTitle || config.zDescription) {
      <header class="flex flex-col space-y-1.5 text-center sm:text-left">
        @if (config.zTitle) {
          <h4 id="z-dialog-title" data-testid="z-title" class="text-lg leading-none font-semibold tracking-tight">{{ config.zTitle }}</h4>

          @if (config.zDescription) {
            <p id="z-dialog-description" data-testid="z-description" class="text-muted-foreground text-sm" [innerHTML]="config.zDescription"></p>
          }
        }
      </header>
    }
  ...
  host: {
    ...
    'role': 'dialog',
    'aria-modal': 'true',
    '[attr.aria-labelledby]': 'config.zTitle ? "z-dialog-title" : null',
    '[attr.aria-describedby]': 'config.zDescription ? "z-dialog-description" : null',
